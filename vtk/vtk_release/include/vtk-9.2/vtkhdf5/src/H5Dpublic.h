 ...
 *          where the attribute, dataset, or named datatype is attached.
 *
 *          The datasets datatype and dataspace are specified by
 ...
 *          properties in the files file access property list.
 *
 ...
 * \param[out] size        Chunk size in bytes, 0 if chunk doesnt exist
 *
 * \return \herr_t
 *
 ...
 *          equal to the datasets rank. Each element is the logical
 *          position of the chunks first element in a dimension.
 *
 * \since 1.10.5
 *
 */
 ...
 * \param[out] offset    Logical position of the chunks first element
 * \param[out] filter_mask Indicating filters used with the chunk when written
 * \param[out] addr      Chunk address in the file
 * \param[out] size      Chunk size in bytes, 0 if chunk doesnt exist
 *
 ...
 * \param[in]  offset   Logical position of the chunks first element in the
 *                      dataspace
 ...
 *          chunk will bypass the librarys internal data transfer
 ...
 *          filter is skipped if the bit corresponding to the filters
 ...
 *          first element of the chunk in the datasets dataspace. The
 ...
 *          file bypassing the librarys internal data transfer pipeline,
 *          including filters.
 *
 *          \p offset is an array specifying the logical position of the
 *          first element of the chunk in the datasets dataspace. The
 ...
 *          the bit corresponding to the filters position in the pipeline
 ...
 * \ingroup H5D
 *
 * \brief Changes the sizes of a datasets dimensions
 *
 * \dset_id
 ...
 *          the rank of the datasets current dataspace.
 *
 ...
 *          the datasets space allocation time is set to #H5D_ALLOC_TIME_EARLY.
 *
 *          Fill values will be written to the dataset in either of the
 *          following situations, but not otherwise:
 *
 *          - If the datasets fill time is set to #H5D_FILL_TIME_IFSET and a
 *            fill value is defined (see H5Pset_fill_time() and
 *            H5Pset_fill_value())
 *          - If the datasets fill time is set to #H5D_FILL_TIME_ALLOC
 *            (see H5Pset_alloc_time())
 *
 * \note If the sizes specified in \p size array are smaller than the datasets
 *       current dimension sizes, H5Dset_extent() will reduce the datasets
 *       dimension sizes to the specified values. It is the user applications
