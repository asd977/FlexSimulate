...
QWidget* MainWindow::buildProjectInfoWidget()
{
    ...
    auto* updatedRow = new QHBoxLayout();
    updatedRow->setSpacing(6);
    updatedRow->setContentsMargins(0, 0, 0, 0);
    auto* updatedTitle = new QLabel(tr("最后修改时间"), timeFrame);
    updatedTitle->setStyleSheet("font-weight:600;color:#1b2b4d;");
    updatedRow->addWidget(updatedTitle);
    updatedRow->addStretch();
    auto* updatedValue = new QLabel(timeFrame);
    updatedValue->setStyleSheet("color:#475569;");
    updatedValue->setTextInteractionFlags(Qt::TextSelectableByMouse);
    updatedRow->addWidget(updatedValue);
    timeLayout->addLayout(updatedRow);

    frameLayout->addWidget(timeFrame);
    layout->addWidget(frame);

    const auto updateTimeLabels = [this, createdValue, updatedValue]() {
        ...
        updatedValue->setText(formatDate(m_projectUpdatedAt));
    };
    updateTimeLabels();
