...
QVector<MainWindow::MaterialRecord> MainWindow::loadMaterialsFromTestData(QString* errorMessage) const
{
    ...
    const QRegularExpression sanitizeExpression(QStringLiteral("[^A-Za-z0-9_\\-]"));
    const auto collectKeys = [&sanitizeExpression](const QStringList& values) {
        QSet<QString> keys;
        for (const QString& value : values)
        {
            ...
            sanitized.replace(sanitizeExpression, QString());
            sanitized = sanitized.trimmed();
            if (!sanitized.isEmpty())
            {
                const QString sanitizedFolded = sanitized.toCaseFolded();
                if (!sanitizedFolded.isEmpty())
                    keys.insert(sanitizedFolded);
            }
        }
        return keys;
    };
    ...
    {
        QFile detailFile(baseDir.filePath(fileName));
        ...
        if (!baseList.isEmpty())
        {
            const QJsonObject entry = baseList.first().toObject();
            const QJsonObject info = entry.value(QStringLiteral("materialBaseInfo")).toObject();
            candidateValues << info.value(QStringLiteral("materialId")).toString();
            candidateValues << info.value(QStringLiteral("gacMaterialTrademark")).toString();
            candidateValues << info.value(QStringLiteral("materialTrademark")).toString();
            candidateValues << info.value(QStringLiteral("id")).toString();
            candidateValues << info.value(QStringLiteral("documentId")).toString();
        }

        const QSet<QString> detailKeys = collectKeys(candidateValues);
        ...
        {
            const QSet<QString> recordKeys = collectKeys(QStringList()
                                                         << record.materialKey
                                                         << record.materialId
                                                         << record.baseId
            ...
            QString applyError;
            if (!applyMaterialDetail(record, detailRoot, &applyError))
            {
                if (errorMessage)
